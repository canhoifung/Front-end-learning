...
继承了HTMLButtonElement接口

实例属性：

### HTMLButtonElement.accessKey

返回一个字符串，表示键盘上对应的键，通过`Alt + 这个键`可以让按钮获得焦点。该属性可读写。

### HTMLButtonElement.autofocus

布尔值，表示页面加载过程中，按钮是否会自动获得焦点。该属性可读写。

### HTMLButtonElement.disabled

布尔值，表示该按钮是否禁止点击。该属性可读写。

### HTMLButtonElement.form

表单元素，返回该按钮所在的表单。该属性只读。如果按钮不属于任何表单，该属性返回`null`。

### HTMLButtonElement.formAction

返回一个字符串，表示表单提交的 URL。该属性可读写，一旦设置了值，点击按钮就会提交到该属性指定的 URL，而不是`<form>`元素指定的 URL。

### HTMLButtonElement.formEnctype

字符串，表示数据提交到服务器的编码类型。该属性可读写，一旦设置了值，点击按钮会按照该属性指定的编码方式，而不是`<form>`元素指定的编码方式。
...
字符串，表示浏览器提交表单的 HTTP 方法。该属性可读写，一旦设置了值，点击后就会采用该属性指定的 HTTP 方法，而不是`<form>`元素指定的编码方法。

### HTMLButtonElement.formNoValidate

布尔值，表示点击按钮提交表单时，是否要跳过表单校验的步骤。该属性可读写，一旦设置会覆盖`<form>`元素的`novalidate`属性。

### HTMLButtonElement.formTarget

字符串，指定了提交了表单以后，哪个窗口展示服务器返回的内容。该属性可读写，一旦设置会覆盖`<form>`元素的`target`属性。

### HTMLButtonElement.label

返回`NodeList`实例，表示那些绑定按钮的`<label>`元素。该属性只读。
...
字符串，表示按钮元素的`name`属性。如果没有设置`name`属性，则返回空字符串。该属性可读写。

### HTMLButtonElement.tabIndex

整数，代表按钮元素的 Tab 键顺序。该属性可读写。

### HTMLButtonElement.type

字符串，表示按钮的行为。该属性可读写，可能取以下的值。
...
字符串，表示没有通过校验时显示的提示信息。该属性只读。

### HTMLButtonElement.validity

返回该按钮的校验状态（`ValidityState`）。该属性只读。
...
布尔值，表示该按钮提交表单时是否将被校验，默认为`false`。该属性只读。



# `<option>`元素

继承了HTMLElement接口和HTMLOptionElement接口
...
若`selected`属性为false，即使`defaultSelected`属性为true也不会被选中
